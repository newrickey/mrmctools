...
task.mrmc<-RJafroc::ReadDataFile(fileName="testJAFROC.xlsx")
jafroc_results<-RJafroc::DBMHAnalysis(task.mrmc, fom="wJAFROC")
```
...
estimates<- prepareestimates(jafroc_results, "B30_200")
...
## For GGPLOT, we are going to reverse the order so the reference is at the top
est$rTreatment <- with(est, factor(Treatment, levels = rev(levels(Treatment))))
```

Plot the figure using the regular ordering of the factor.
```{r}
...
This is a repeat of the above figure only this time electing to use the RRRC model.
```{r}
plotFOM(est, "RRRC", "rTreatment")
```


Each of the plot objects returns a ggplot2 object that can be saved using the figuresave function.

```{r}
pfom_rrrc <- plotFOM(est, "RRRC", "rTreatment")
figuresave("fig1_fom_rrrc.png",pfom_rrrc)
```


Note: The figures for RRRC and FRRC will look very similar in most cases. The CIs will be slightly different as they are based on different variance estimation approaches. It's a good idea to use descriptive file names to help keep the various versions straight, particularly if multiple jafroc files are generated based on different threshold values. 


## Delta FOM Plots

Repeat the above now with delta FOM
...
### Notice the delta FOM figure has some additional options
delta_fom_rrrc<-plotDeltaFOM(del,"RRRC","FBP 200 QRM",-.05,thetreatment = "rTreatment")
```

Plot of the Delta FOMs
...
The package's functions are still being tweaked. Use is evolving, and user feedback is appreciated. 
